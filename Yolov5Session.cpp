...
{
	return CreateSession(modelPath) && ParasModel() && WarmUpModel();
}
...
std::vector<ResultNode> Yolov5Session::Detect(const cv::Mat& image)
{
	...
	if (processor_)
	{
		auto inputTensor = processor_->Preprecess(image);
...
{
	if (!std::filesystem::exists(modelPath)) //如果不存在这么模型路径 将返回false
		return false;
	envName_ = modelPath.filename().string();
	env_ = Ort::Env(OrtLoggingLevel::ORT_LOGGING_LEVEL_WARNING, envName_.c_str()); //什么意思

	sessionOpt = Ort::SessionOptions();
	sessionOpt.SetInterOpNumThreads(0);
