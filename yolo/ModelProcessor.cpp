	...
	:model_(model)
{
	...
	if (!shapes.empty())
	{
		...
		if (shape.size() == 4)
		{
			...
			blob_ = new float[blobSize_]; //什么语法
		}
	}
...
std::vector<Ort::Value> ModelProcessor::Preprecess(const cv::Mat& image)
{
	std::vector<Ort::Value> result;  //定义一个保存结果的向量
	...
	try
	{
		...
		auto inputTensorShape = inputShape[0];   //yolo只有一个 维度输入

		if (!ConvertToRGB(image, resizeImage))
			throw std::runtime_error("failed to convert to rgb!");

		//归一化为统一大小
		resizeImage = Letterbox(resizeImage, cv::Size(inputTensorShape.at(2), inputTensorShape.at(3)));

		//映射0~255到0~1之间
		resizeImage.convertTo(floatImage, CV_32FC3, 1 / 255.0);
...
std::vector<ResultNode> ModelProcessor::Postprocess(const std::vector<Ort::Value>& outTensor, const cv::Size& originalImageShape, float confThreshold, float iouThreshold)
{
...
cv::Mat ModelProcessor::Letterbox(const cv::Mat& image, const cv::Size& newShape, const cv::Scalar& color, bool scaleFill, bool scaleUp, int stride)
{
	//参数验证
	if (newShape.width <= 0 || newShape.height <= 0)
		throw std::invalid_argument("Invalid new Shape dimensions.");
	...
	//调整图像大小
	if (shape.width != static_cast<int>(shape.width * scale_factor) && shape.height != static_cast<int>(shape.height * scale_factor))
	{
		cv::resize(image, outImage, cv::Size(static_cast<int>(shape.width * scale_factor), static_cast<int>(shape.height * scale_factor)));
	}
	else
	{
		outImage = image;
...
{
	if (image.empty())  //判断是否为空
		return false;
	
	int channel = image.channels();
	if (channel == 1)  //灰度图
		cv::cvtColor(image, outImage, cv::COLOR_GRAY2BGR);
	else if (channel == 3)
...
void ModelProcessor::GetOriCoords(const cv::Size& currentShape, const cv::Size& originalShape, cv::Rect& outCoords)
{
...
void ModelProcessor::GetBestClassInfo(std::vector<float>::iterator it, const int& numClasses, float& bestConf, int& bestClassId)
{
...
void ModelProcessor::ParseRawOutput(const std::vector<Ort::Value>& tensor, float conf_threshold, std::vector<cv::Rect>& boxes, std::vector<float>& confs, std::vector<int>& classIds)
{
